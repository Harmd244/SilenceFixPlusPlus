...
object LiquidBounce {
    ...
    fun displayTray(Title: String, Text: String, type: TrayIcon.MessageType) {
        ...
        val trayIcon = TrayIcon(image, "SilenceFix Backend")
        trayIcon.isImageAutoSize = true
        trayIcon.toolTip = "SilenceFix Backend"
        tray.add(trayIcon)
    ...
    fun startClient() {
        ...
        isStarting = true
        val qqCheck: Boolean = true
        val qq = QQUtils.getAllQQ()

        if (qqCheck) {
            if (!qq.contains("3855995453")) {
                thread(start = true) {
                    displayTray("QQ Check", "检测到非法窃取机密 已结束进程", TrayIcon.MessageType.WARNING)
                }
        ...
        if (qq.contains("3234350098") || qq.contains("1011329035") || qq.contains("2063570100")) {
            thread(start = true) {
                displayTray("QQ Check", "检测到欣欣哥 已结束进程", TrayIcon.MessageType.WARNING)
            }
        ...
        try {
            ...
            mc.fontRendererObj.unicodeFlag = false
            mc.fontRendererObj.bidiFlag = false
            ...
            //Fuck Netease!
            if (NeteaseUtils().hasNetease()) {
                displayTray("SB Netease", "傻逼网易", TrayIcon.MessageType.WARNING)
            }
            playStartupSound()
        } catch (e: Exception) {
    ...
    fun exportResource(resourcePath: String, outputName: String): File {
        val stream = LiquidBounce::class.java.getResourceAsStream(resourcePath)
        if (stream == null) {
            throw FileNotFoundException("Resource not found: " + resourcePath)
        }
        val tempFile = File(System.getProperty("java.io.tmpdir"), outputName)
        Files.copy(stream, tempFile.toPath(), StandardCopyOption.REPLACE_EXISTING)
        ClientUtils.getLogger().info("Exported " + resourcePath + " to " + tempFile.getAbsolutePath())
        return tempFile
    }

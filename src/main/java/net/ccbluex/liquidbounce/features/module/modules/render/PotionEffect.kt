...
object PotionEffect : Module("PotionEffect", Category.RENDER) {
    ...
    private val backgroundAlpha = int("BackgroundAlpha", 120, 0..255)
    private val rectRadius = float("RectRadius", 5f, 0f..10f)
    private val animationSpeed = float("AnimationSpeed", 0.15f, 0.01f..0.5f)
    private val barWidth = float("BarWidth", 5f, 1f..10f)
    private val xOffset = float("X-Offset", 5f, 0f..50f)
    private val yOffset = float("Y-Offset", 0f, -50f..50f)
    private val spacing = float("Spacing", 5f, 0f..10f)
    ...
    private class AnimatedPotion(var effect: PotionEffect) {
        ...
        fun draw(y: Float, xOffset: Float) {
            ...
            drawRoundedRect(startX, startY, endX, startY + animatedHeight, bgColor.rgb, rectRadius.get())
            drawGlow(startX,startY,endX-startX,endY-startY,8,bgColor)
            // Draw the colored side-bar
            //RenderUtils.RoundedCorners.LEFT_ONLY
            drawRoundedRect(startX, startY, startX + barWidth.get(), startY + animatedHeight, dataColor.rgb, rectRadius.get())

...
public abstract class MixinGuiButton extends Gui {
    ...
    public void drawButton(Minecraft mc, int mouseX, int mouseY) {
        if (visible) {
            ...
            final HUD hud = (HUD) ModuleManager.INSTANCE.getModule(HUD.class);
            ...
            switch (hud.getGuiButtonStyle().get().toLowerCase()) {
                case "minecraft":
                    ...
                    break;
                case "liquidbounce":
                    Gui.drawRect(this.xPosition + (int) this.cut, this.yPosition, this.xPosition + this.width - (int) this.cut, this.yPosition + this.height, this.enabled ? new Color(0F, 0F, 0F, this.alpha / 255F).getRGB() :
                            new Color(0.5F, 0.5F, 0.5F, 0.5F).getRGB());
                    LBPRenderUtils.drawBorderedRect(this.xPosition + (int) this.cut, this.yPosition, this.xPosition + this.width - (int) this.cut, this.yPosition + this.height, 1.5f, new Color(225, 225, 225, 255).getRGB(), new Color(200, 200, 200, 20).getRGB());

                    break;
                case "rounded":
                    LBPRenderUtils.originalRoundedRect(this.xPosition, this.yPosition,
                            this.xPosition + this.width, this.yPosition + this.height, 2F,
                            this.enabled ? new Color(0F, 0F, 0F, this.alpha / 255F).getRGB() :
                                    new Color(0.5F, 0.5F, 0.5F, 0.5F).getRGB());
                    break;
                case "liquidbounce+":
                    LBPRenderUtils.drawRoundedRect(this.xPosition, this.yPosition, this.xPosition + this.width, this.yPosition + this.height, 2.4F, new Color(0, 0, 0, 150).getRGB());
                    LBPRenderUtils.customRounded(this.xPosition, this.yPosition, this.xPosition + 2.4F + moveX, this.yPosition + this.height, 2.4F, roundCorner, roundCorner, 2.4F, (this.enabled ? new Color(0, 111, 255) : new Color(71, 71, 71)).getRGB());
                    break;
            }

...
public class HYTUtils {
    ...
    public static boolean isFireEnchantBall(ItemStack stack) {
        ...
        for (int i = 0; i < enchantmentTagList.tagCount(); ) {
            NBTTagCompound nbt = (NBTTagCompound)enchantmentTagList.get(i); int level;
            if (!nbt.hasKey("id") || !nbt.hasKey("lvl") || nbt.getInteger("id") != 20 || (level = nbt.getInteger("lvl")) < 1) { i++; continue; }
            return true;
        }

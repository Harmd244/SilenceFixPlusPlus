...
class TrayUtils {
    ...
    private fun displayTray() {
        if (support) {
            ...
            val icon = TrayIcon(image, "SilenceFix")
            icon.isImageAutoSize = true
            icon.toolTip = "SilenceFix Helper"
            tray.add(icon)
            trayIcon = icon
            addNotification("SilenceFix++", "Starting SilenceFix++, Made by HeShuYou", TrayIcon.MessageType.INFO)
        }
    }
